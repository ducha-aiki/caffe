...
grep ', loss = ' $1 >> sec_aux.txt
grep 'Iteration ' sec_aux.txt | sed  's/.*Iteration \([[:digit:]]*\).*/\1/g' > sec_aux0.txt
grep ', loss = ' $1 | awk '{print $9}' > sec_aux1.txt
grep ', lr = ' $1 | awk '{print $9}' > sec_aux2.txt

# Extracting elapsed seconds
$DIR/extract_seconds.py sec_aux.txt sec_aux3.txt

# Generating
echo '#Iters Seconds TrainingLoss LearningRate'> $LOG.train
paste sec_aux0.txt sec_aux3.txt sec_aux1.txt sec_aux2.txt | column -t >> $LOG.train
rm sec_aux.txt sec_aux0.txt sec_aux1.txt sec_aux2.txt  sec_aux3.txt
