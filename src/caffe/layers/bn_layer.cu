...
namespace caffe {
  ...
  void BNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    // EX across spatial
    caffe_gpu_gemv<Dtype>(CblasNoTrans, N_ * C_, H_ * W_, Dtype(1. / (H_ * W_)), bottom_data,
    ...
    caffe_gpu_add(buffer_blob_.count(), bottom_data, buffer_blob_.gpu_data(), top_data);
    ...
    caffe_gpu_div(buffer_blob_.count(), top_data, buffer_blob_.gpu_data(), top_data);
    ...
    // scale
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Dtype(1),
        batch_sum_multiplier_.gpu_data(), this->blobs_[0]->gpu_data(), Dtype(0),
        spatial_variance_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_ * C_, H_ * W_, 1, Dtype(1),
        spatial_variance_.gpu_data(), spatial_sum_multiplier_.gpu_data(), Dtype(0),
        buffer_blob_.mutable_gpu_data());

    caffe_gpu_mul(buffer_blob_.count(), top_data, buffer_blob_.gpu_data(), top_data);

    // shift
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_, C_, 1, Dtype(1),
        batch_sum_multiplier_.gpu_data(), this->blobs_[1]->gpu_data(), Dtype(0),
        spatial_mean_.mutable_gpu_data());
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_ * C_, H_ * W_, 1, Dtype(1),
        spatial_mean_.gpu_data(), spatial_sum_multiplier_.gpu_data(), Dtype(0),
        buffer_blob_.mutable_gpu_data());
    caffe_gpu_add(buffer_blob_.count(), top_data, buffer_blob_.gpu_data(), top_data);
  ...
  void BNLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    caffe_gpu_mul(buffer_blob_.count(), x_norm_.gpu_data(), top_diff, buffer_blob_.mutable_gpu_data());
//		caffe_gpu_mul(buffer_blob_.count(), top_data, top_diff, buffer_blob_.mutable_gpu_data());
    // EX across spatial
    ...
    caffe_gpu_gemv<Dtype>(CblasTrans, N_, C_, Dtype(1), spatial_variance_.gpu_data(),
        batch_sum_multiplier_.gpu_data(), Dtype(0), this->blobs_[0]->mutable_gpu_diff());

    // gradient w.r.t. shift
    // EX across spatial
    caffe_gpu_gemv<Dtype>(CblasNoTrans, N_ * C_, H_ * W_, Dtype(1), top_diff,
        spatial_sum_multiplier_.gpu_data(), Dtype(0), spatial_mean_.mutable_gpu_data());
    // EX across batch
    caffe_gpu_gemv<Dtype>(CblasTrans, N_, C_, Dtype(1), spatial_mean_.gpu_data(),
        batch_sum_multiplier_.gpu_data(), Dtype(0), this->blobs_[1]->mutable_gpu_diff());
    ...
    caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, N_ * C_, H_ * W_, 1, Dtype(1),
        spatial_variance_.gpu_data(), spatial_sum_multiplier_.gpu_data(), Dtype(0),
        buffer_blob_.mutable_gpu_data());
    caffe_gpu_mul(buffer_blob_.count(), top_diff, buffer_blob_.gpu_data(), buffer_blob_.mutable_gpu_data());

    // use new top diff for computation
    caffe_gpu_mul(buffer_blob_.count(), x_norm_.gpu_data(), buffer_blob_.gpu_data(), bottom_diff);
    // EX across spatial
    caffe_gpu_gemv<Dtype>(CblasNoTrans, N_ * C_, H_ * W_, Dtype(1), bottom_diff,
        spatial_sum_multiplier_.gpu_data(), Dtype(0), spatial_mean_.mutable_gpu_data());
    // EX across batch
    caffe_gpu_gemv<Dtype>(CblasTrans, N_, C_, Dtype(1), spatial_mean_.gpu_data(),
        batch_sum_multiplier_.gpu_data(), Dtype(0), batch_mean_.mutable_gpu_data());
    ...
    caffe_gpu_mul(buffer_blob_.count(), x_norm_.gpu_data(), bottom_diff, bottom_diff);
    ...
    caffe_gpu_div(buffer_blob_.count(), bottom_diff, buffer_blob_.gpu_data(), bottom_diff);
  }
