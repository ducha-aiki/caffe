...
__global__ void EltwiseAffineForward(const int n, const int channels,
...
template <typename Dtype>
__global__ void EltwiseAffineBackward(const int n,
...
template <typename Dtype>
__global__ void EltwiseAffineParamSlopeBackward(const int n,
...
template <typename Dtype>
void EltwiseAffineLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  EltwiseAffineForward<Dtype><<<CAFFE_GET_BLOCKS(count),
...
template <typename Dtype>
void EltwiseAffineLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[1]) {
    ...
    // Gradient with respect to bias
      for (int n = 0; n < num; ++n) {
    caffe_gpu_gemv<Dtype>(CblasTrans, channels, height*width, (Dtype)1.,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    for (int n = 0; n < bottom[0]->num(); ++n) {
      ...
      // NOLINT_NEXT_LINE(whitespace/operators)
      EltwiseAffineParamSlopeBackward<Dtype><<<CAFFE_GET_BLOCKS(cdim),
          CAFFE_CUDA_NUM_THREADS>>>(
  ...
  if (propagate_down[0]) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    EltwiseAffineBackward<Dtype><<<CAFFE_GET_BLOCKS(count),
        CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(EltwiseAffineLayer);
