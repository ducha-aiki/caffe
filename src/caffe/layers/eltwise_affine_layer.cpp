...
void EltwiseAffineLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  CHECK_GE(bottom[0]->num_axes(), 2)
      << "Number of axes of bottom blob must be >=2.";
  EltwiseAffineParameter eltwise_affine_param =
      this->layer_param().eltwise_affine_param();
  ...
  if (this->blobs_.size() > 0) {
    ...
    if (eltwise_affine_param.has_slope_filler()) {
      filler.reset(GetFiller<Dtype>(eltwise_affine_param.slope_filler()));
    } else {
    ...
    if (eltwise_affine_param.has_bias_filler()) {
      filler.reset(GetFiller<Dtype>(eltwise_affine_param.bias_filler()));
    } else {
      FillerParameter filler_param;
      filler_param.set_type("constant");
  ...
  caffe_set(multiplier_.count(), Dtype(1.0), multiplier_.mutable_cpu_data());
}

template <typename Dtype>
void EltwiseAffineLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  CHECK_GE(bottom[0]->num_axes(), 2)
      << "Number of axes of bottom blob must be >=2.";
...
template <typename Dtype>
void EltwiseAffineLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void EltwiseAffineLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(EltwiseAffineLayer);
#endif

INSTANTIATE_CLASS(EltwiseAffineLayer);
