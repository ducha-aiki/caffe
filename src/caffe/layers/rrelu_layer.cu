...
__global__ void RReLUForwardTrain(const int n, const int channels, const int dim,
...
// CUDA kernele for forward
template <typename Dtype>
__global__ void RReLUForwardTest(const int n, const int channels, const int dim,
    const Dtype* in, Dtype* out, const Dtype slope_data) {
  CUDA_KERNEL_LOOP(index, n) {
...
template <typename Dtype>
__global__ void RReLUBackward(const int n, const int channels, const int dim,
...
void RReLULayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TRAIN) {
	...
	RReLUForwardTrain<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, channels, dim, bottom_data, top_data, slope_data);
    CUDA_POST_KERNEL_CHECK;
  } else {
    RReLUForwardTest<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
void RReLULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    RReLUBackward<Dtype><<<CAFFE_GET_BLOCKS(count),
