...
void ImageLocDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  while (infile >> filename >> label >> x_min >> y_min >> x_max >> y_max) {
      std::vector<int> labels1(5);
      labels1[0] = label;
      labels1[1] = x_min;
      labels1[2] = y_min;
      labels1[3] = x_max;
      labels1[4] = y_max;
    lines_.push_back(std::make_pair(filename, labels1));
  }
...
void ImageLocDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    prefetch_label[item_id*5] = lines_[lines_id_].second[0];
    int x_min = lines_[lines_id_].second[1];
    int y_min = lines_[lines_id_].second[2];
    int x_max = lines_[lines_id_].second[3];
    int y_max = lines_[lines_id_].second[4];
