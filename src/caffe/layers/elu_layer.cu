...
__global__ void ELUForward(const int n, const Dtype* in, Dtype* out,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    out[index] = in[index] > 0 ? in[index] : (exp(in[index]) - Dtype(1)) * negative_slope;
  }
}
...
__global__ void ELUBackward(const int n, const Dtype* in_diff,
    const Dtype* in_data,const Dtype* out_data, Dtype* out_diff, Dtype negative_slope) {
  CUDA_KERNEL_LOOP(index, n) {
    out_diff[index] = in_diff[index] * ((in_data[index] > 0)
        + (in_data[index] <= 0) * (out_data[index] + negative_slope));
  }
}
...
void ELULayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    ELUBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff, bottom_data, top_data, bottom_diff, negative_slope);
    CUDA_POST_KERNEL_CHECK;
  }
}
